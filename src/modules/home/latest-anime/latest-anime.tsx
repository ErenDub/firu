...
export const LatestAnime = () => {
  const $getLatestAnimes = useQuery(`latest-anime`, () =>
    getLatest({ limit: 15, type: "ანიმე" })
  );
  ...
  return (
    <Box mb={10}>
      {$getLatestAnimes.isLoading && <Skeleton height={400} />}
      ...
      <Swiper {...swiperParams} className="mySwiper">
        {$getLatestAnimes.data &&
          $getLatestAnimes.data.movies.map((movie) => (
