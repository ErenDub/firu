...
export const LatestTvShows = () => {
  const $getLatestTvShows = useQuery(`latest-tv-shows`, () =>
    getLatest({ limit: 15, type: "სერიალი" })
  );
  ...
  return (
    <Box mb={10}>
      {$getLatestTvShows.isLoading && <Skeleton height={400} />}
      <Stack
        direction="row"
      ...
      <Swiper {...swiperParams} className="mySwiper">
        {$getLatestTvShows.data &&
          $getLatestTvShows.data.movies.map((movie) => (
